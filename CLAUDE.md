...
QMind 是 **CLI 工具 + HTML 报告**，不是 GUI 桌面程序，不是 Web 服务。

```
你打开终端跑命令 → 终端看分析结果 → 信号推到飞书 → 报告存成 HTML
```
...
| 场景            | 命令                                            | 输出                                                   | 频率      |
| --------------- | ----------------------------------------------- | ------------------------------------------------------ | --------- |
| 一次性分析/回测 | `qmind analyze BTC/USDT` / `qmind backtest` | 终端结构化 JSON + HTML 报告（P&L曲线/Sharpe/分段绩效） | 按需      |
| 持续监控        | `qmind watch BTC/USDT`（常驻进程）            | 飞书/邮件推送信号 + 终端通知                           | 全天候    |
| 事后学习        | `qmind learn --from-log trades.log`           | 终端教训总结 + 经验库更新                              | 每周/每月 |
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 QMind 分析报告 · 2026-06-30 08:00 UTC
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 BTC/USDT  当前: 86,200
 │                         技术面: 看多 (0.78)
 │ 分析师共识 →  基本面: 中性 (0.52)
 │              情绪面: 看多 (0.65)
 ─────────────────────────────────────
 辩论分歧度: δ=0.13 → 低分歧，直接采信
 ─────────────────────────────────────
 决策: LONG at 86,200
 止损: 85,100 (-1.28%)
 目标: 89,500 (+3.83%)
 仓位: 12.5% (calibrated confidence: 0.61)
 风险收益比: 2.99
 ─────────────────────────────────────
 ⚡ dryRun 模式 — Net PnL 已模拟: -$0.00
 命令: qmind analyze BTC/USDT --live  # 去掉 dryRun 实盘
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
$ qmind backtest --strategy ma_cross --start 2024-01 --end 2025-06
  → 输出 reports/ma_cross_2024-01_2025-06.html
  → 浏览器打开，看 PnL 曲线、Sharpe、分段绩效、成本敏感性
```
...
│  │  CLI（必需）       │  │  HTML 报告（必需）              │   │
│  │                   │  │                               │   │
│  │  qmind analyze    │  │  PnL 曲线图                   │   │
│  │  qmind backtest   │  │  分段绩效表                    │   │
│  │  qmind watch      │  │  成本敏感性分析                │   │
│  │  qmind learn      │  │  消融实验对比                  │   │
│  └──────────────────┘  └───────────────────────────────┘   │
│                                                             │
│  ┌──────────────────┐  ┌───────────────────────────────┐   │
│  │  通知（推荐）      │  │  Web Dashboard（可选）         │   │
│  │                   │  │                               │   │
│  │  飞书 Webhook     │  │  实时 Agent 决策过程可视化     │   │
│  │  邮件 SMTP        │  │  交易历史 + 教训记录           │   │
│  └──────────────────┘  └───────────────────────────────┘   │
│                                                             │
└───────────────────────────┬─────────────────────────────────┘
...
┌─────────────────────────────────────────────────────────────┐
│                   核心引擎（CLI 驱动）                        │
│  agents/ → graph/ → execution/ → learning/                  │
│  所有逻辑独立于交互方式，核心是 Python 包                     │
└─────────────────────────────────────────────────────────────┘
```

### 不是什么

- ❌ 不是桌面 GUI（PA_Agent 已经是 PyQt6，QMind 不重复）
- ❌ 不是手机 App（飞书推送足够了）
- ❌ 不是交互式交易终端（不是要替代交易所）
- ❌ 不是 SaaS 平台（不提供多人共享服务）

### Phase 验收标准（补充）

- **Phase 3 结束时**：`qmind analyze BTC/USDT --dry-run` 跑通，输出完整决策 JSON
- **Phase 4 结束时**：`qmind learn --from-log trades.log` 输出可验证的教训总结
- **Phase 5 结束时**：`qmind watch BTC/USDT` 连续运行 7 天不中断，有信号推飞书
...
所有 Phase **编码完成**。621 测试全通过（19.75s），~18,600 行总代码量。

```
Phase 0: 基础设施 (2 周)
  [x] 0.1  Python 工程化：pyproject.toml / ruff / pytest / pre-commit
  [x] 0.2  LLM 统一调用层：client / structured / cost_tracker / router
  [x] 0.3  基础数据模型：state / MarketData / protocol schemas / time_guard

Phase 1: 单 Agent 强基线 (3 周)
  [x] 1.1  LLM 决策 Agent + Financial CoT prompt
  [~] 1.2  数据源适配：yfinance / Tushare 已完成，AkShare 待接入
  [x] 1.3  回测框架：walk-forward partition / cost_model / calibration
  [x] 1.4  工具层：market_data / backtest / portfolio

Phase 2: 多 Agent 协作 (3 周)
  [x] 2.1  4 分析师 prompt + 并行调度
  [x] 2.2  LangGraph 五阶段图 + 条件路由（含 TimeGuard + CVRF 节点）
  [x] 2.3  修正版辩论系统：分歧检测 / Trust / Skeptic / Leader
  [x] 2.4  三角风控 + CVaR 硬约束
  [x] 2.5  消融实验：单 Agent vs 多 Agent vs 辩论 / 成本追踪 / 相似度

Phase 3: 执行层 + 策略 (3 周)
  [x] 3.1  交易所适配：Binance / OKX / Bybit + 下单校验器
  [x] 3.2  策略工厂：基类 + 注册表 + 内置 17 策略（计划 22+）
  [~] 3.3  TiMi 解耦：LLM 策略生成器 + 代码编译器 待实现

Phase 4: CVRF 学习 (3 周)
  [x] 4.1  交易评估 + CVRF 反思 prompt
  [x] 4.2  概念记忆库 SQLite + 相似度检索 + 教训注入
  [x] 4.3  回测 → 教训 → 记忆 → 下一窗口 闭环（已接入 pipeline）

Phase 5: 部署 + 监控 (2 周)
  [x] 5.1  配置管理 + CLI 入口 + dryRun 模式
  [x] 5.2  全链路审计日志 + P1-P6 报告 + 偏差检查器
  [x] 5.3  定时调度 + 邮件/飞书通知 + 多品种并行
```
...
- [x] **Phase 0 结束时**：`pip install -e . && make test && make lint` 一键跑通，LLM 调用成本可追踪
- [ ] **Phase 1 结束时**：单 Agent 在回测窗口输出可复现的 Net PnL，ECE ≤ 0.05（需实跑回测）
- [ ] **Phase 2 结束时**：消融实验报告证明多 Agent 的净增量贡献（需实跑回测）
- [x] **Phase 3 结束时**：dryRun 模式下完整跑通 分析 → 辩论 → 决策 → 风控 → 模拟执行
- [x] **Phase 4 结束时**：经验库有 > 100 条教训条目，回测窗口 2 比窗口 1 有可检测的绩效提升（已接入 pipeline）
- [ ] **Phase 5 结束时**：系统能以 `qmind --mode live --dry-run` 连续运行 7 天不中断（需实测）
